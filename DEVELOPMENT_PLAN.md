...
| `map_drill` | `path` + (`name` \| `from`/`to`) | padded definition span (±8 ctx lines) or exact line window; credits the ledger |
...
8. **`map_drill` tool:** returns one definition's full span or a line window of a file — and
   credits the read-before-edit ledger (§6.3). **DECIDED (2026-09-11):** definition spans are
   padded with the step-5 context constant (±8 lines, clamped at file edges, shared with the
   renderer so zoom matches the overview) — the ledger credit makes the drill the model's
   possible whole view of a region, and the lines just outside a span (doc comments,
   attributes, `impl` headers, item boundaries) are what a SEARCH block anchors on; explicit
   `from`/`to` windows are the model's own choice and are never padded.
...
> **Erratum (2026-09-14).** "Keeping the last 3 turns verbatim" and "fitting the
> context window" are not jointly satisfiable: three `read` observations at the
> §6.1 cap are ~65k estimated tokens against the §7 default 32k window, and with
> three turns or fewer there is nothing to summarize. The implementation returned
> the history unchanged and the oversized request shipped, because nothing
> downstream measured it — over HTTP that is worse than an error, since
> llama-server and Ollama commonly truncate from the *left*, discarding the system
> prompt and every phase gate with it. Verbatim is therefore bounded: when no turn
> can be dropped, the largest observations are clipped (never below 256 tokens,
> marked in the text) exactly as a sub-coder clips its own (§6.8), so every turn
> stays present with its evidence bounded. The agent additionally refuses to ship
> silently: an assembled request over the window is reported with a remedy.
...
- `tokio` (rt-multi-thread, macros, sync, fs, io-util, process, signal — `signal`
  added at M8: §6.1 step 5 mandates Ctrl-C stream abort; `io-util` added at the
  2026-09-14 remediation: §6.1's output caps must bound *memory*, so both
  subprocess call sites drain their pipes incrementally through
  `rusta_core::proc::capped_read` instead of buffering with `wait_with_output`) · `reqwest` 0.12 (rustls-tls, json, stream;
  default-features off) · `serde`/`serde_json`/`toml` (config & tool-call args only — never the
  edit format) · `clap` 4 (derive) · `anyhow` + `thiserror` · `reedline` (REPL; default features
  off — only the optional helix keymap is dropped) · `futures` (streams)
...
- `regex` — case-sensitive regexes for the `grep` tool (§6.4) and the shell deny-list (§6.12);
  both sections mandate regex semantics, so it earns its place twice (added at M7).
...
**Audit result (2026-09-11, M8 closed):** every row below ships and is tested; milestone
citations point at the §8 evidence trails.
