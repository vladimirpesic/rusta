...
| **A37** | F-15 | **Status: RESOLVED** (wave 4). Input-validation asymmetry: `{"task": ""}` bypasses the non-empty check the `tasks` array form enforces (`from_items` filters `!t.trim().is_empty()`; the single-task arm does not), spawning a sub-coder with an empty brief. | `rusta-dispatch/src/dag.rs:77` vs `99-103` |
| **A38** | F-16 | **Status: RESOLVED** (wave 4) — clipped, not skipped: a mitigation note that silently fails to ship would be an inert safeguard. `capsule_note`'s budget guard is `> CAPSULE_TOKEN_BUDGET && included > 0`, so a *first* capsule over 180 tokens ships regardless — the §6.6 guarantee has a hole. Latent: the shipped set's longest text is ~25 tokens. | `rusta-core/src/context.rs:1122-1126` |
| **A39** | F-17 | **Status: RESOLVED** (wave 4). Module doc says "Four states, **six** scaffold events" — `PHASE_EVENTS: [PhaseEvent; 7]` since `LoopEscalated`. | `rusta-core/src/state.rs:3` |
| **A40** | F-18 | **Status: RESOLVED** (wave 4) — the real figures are 28 cells, 9 legal edges, 1 no-op, 18 illegal. The M3 acceptance test's own doc says "all **24** `(State × PhaseEvent)` cells … all **sixteen** illegal combinations"; the table is 4 × 7 = **28**. Assertions are correct; only the prose miscounts. With A39, both load-bearing spots in `state.rs` miscount the table they exist to pin. | `rusta-core/src/state.rs:525-527` |
| **A41** | F-23 | **Status: RESOLVED** (wave 4) by building the missing half rather than weakening the claim: the corpus now applies every fixture representing an applicable edit, with the three exceptions listed in-test with reasons. A 16th fixture (glued marker) was added because a probe showed the corpus caught nothing when §6.3's marker strip was disabled. ADR §9 says each corpus fixture carries "an expected parse **and** an expected apply result". `parser_corpus_is_green` is the only fixture-driven test and calls **only** `parse_response`; no fixture is ever run through `Editor`. The `Editor` uses in `corpus.rs` are hand-written cases. | `rusta-edit/tests/corpus.rs:155-195` vs `ADR.md:735-738` |
...
| **A45** | F-29 | **Status: RESOLVED** (wave 4). `Config::summary()` journals `self.backend.kind` — the **file's** value — ignoring the `--backend` override that the adjacent accessor honours. One `SessionStart` event can record `backend=http` beside an embedded backend. | `rusta-cli/src/config.rs:240-250` |
| **A46** | F-30 | **Status: RESOLVED** (wave 4). ADR §9 locates the parser corpus at `crates/rusta-edit/tests/edit_corpus/`; it lives at workspace-root `tests/edit_corpus/`. Loaders point at the real path; the normative document does not. | `ADR.md:735` |
| **A47** | F-31 | **Status: RESOLVED** (wave 4) — documented in ADR §7 and `rusta.toml.example`. `[validate] timeout_secs` is parsed and validated but appears in neither ADR §7's schema nor `rusta.toml.example` (whose `timeout_secs` is under `[shell]`). The inverse of the `strict_grammar` archetype: real, working, undocumented. | `rusta-validate/src/validators.rs:84-89` |
| **A48** | F-32 | **Status: RESOLVED** (wave 4) — the gate now scans workspace-root `tests/`, `benches/` and `examples/` when present. The §12 gate scans only `crates/`; any `.rs` under workspace-root `tests/`, `benches/` or `examples/` escapes R1 counting. Harmless today (root `tests/` is `.md`-only), but §12's contract says "counted on `*.rs`" with no such boundary. | `scripts/loc_budget.sh:30,41` |
| **A49** | F-33 | **Status: RESOLVED** (wave 4). ADR §5's layout comment describes the root manifest as "features: embedded, embedded-cuda (opt-in)"; the root `Cargo.toml` defines **no** `[features]` section — they live on the member crates. | `ADR.md:140` |
| **A50** | F-34 | **Status: RESOLVED** (wave 4) — a `Docs` step with `RUSTDOCFLAGS: -D warnings`, verified to pass before being added. **CI has no `cargo doc` step** (it runs fmt, clippy, test, loc_budget on the default graph and test+clippy on the embedded graph), yet ADR §17 reports "`cargo doc` 0 warnings" as a gate and ~650 doc comments cite ADR sections. Nothing prevents doc-warning drift. | `.github/workflows/ci.yml` |
| **A51** | F-35 | **Status: RESOLVED AS DOCUMENTED, not changed** — see §5.6; the premise was wrong. `multiple-versions = "warn"` — a dependency duplication warns but never fails CI. Moot today. | `deny.toml:26` |
| **A52** | F-10 | **Status: RESOLVED** (wave 4). The parity justification "TSX shares TypeScript's tags query (**Aider does the same**)" is unsupported: Aider's query packs contain `typescript-tags.scm` and no TSX query, and `repomap.py` does not mention `tsx` at all. Rusta's choice is a reasonable superset and breaks no ADR rule; the cited justification is false. | `rusta-repomap/src/lang.rs:70` |
| **A53** | F-12 | **Status: RESOLVED** (wave 4). Stale comment: "the timeout's drop of the `wait_with_output` future guarantees the child is killed" — the code builds a custom `collect` future; `wait_with_output` appears only in comments. | `rusta-tools/src/shell.rs:302` |
...
**5.6 — A51: "currently moot (lockfile has no duplicate name+version pairs)".** The conclusion
does not follow, because a lockfile cannot contain duplicate name+version pairs — that tuple is
its primary key. Counting duplicate *names* instead finds five: `getrandom`, `itertools`,
`shlex`, `syn` and `windows-sys`. So `multiple-versions = "deny"` would fail CI immediately, on
duplicates that are transitive and not ours to unify. The finding is real (the line does not
gate) but its remedy is not; `deny.toml` now records the measurement and why the line reads
`warn`.
